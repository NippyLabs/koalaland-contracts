module nippy_pool::underlying_token_factory {
    use std::error;
    ...
    public entry fun create_token(
        ...
        let coin_list = borrow_global_mut<CoinList>(@nippy_pool);
    ...
    public fun assert_token_exists(token_metadata_address: address) acquires CoinList {
        let coin_list = borrow_global<CoinList>(@nippy_pool);
        assert!(smart_table::contains(&coin_list.value, token_metadata_address),
            E_TOKEN_ALREADY_EXISTS);
    }
    ...
    public fun get_metadata_by_symbol(symbol: String): Object<Metadata> {
        let metadata_address =
            object::create_object_address(&@underlying_tokens_admin, *string::bytes(&symbol));
    ...
    fun only_token_admin(account: &signer) {
        assert!(signer::address_of(account) == @underlying_tokens_admin, ENOT_OWNER)
    }
